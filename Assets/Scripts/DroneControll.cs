...
public class DroneControll : MonoBehaviour
{
    ...
    void Awake()
    {
           if (!hasSpawnCompleteListener)
        {
            Random_Target.OnSpawnComplete += HandleSpawnComplete;
            hasSpawnCompleteListener = true;
        }
    }
    ...
    void Start()
    {
        ...
        {
            float dist = CalculateDistance(position, dronePosition);
            distances.Add(dist);

            if (distances.Count == 1)
            {
                en_yakinPos = position;
            }
            else
            {
                if (dist < CalculateDistance(en_yakinPos, dronePosition))
                {
                    en_yakinPos = position;
                }
            }
...
void HandleSpawnComplete()
    ...
    private void sweep(float sweepangle)
    {
     
        // Mevcut rotasyonu alın
        Quaternion currentRotation = rb.rotation;

        // Yeni rotasyonu belirlemek için Euler açılarını kullanın
        Quaternion newRotation = Quaternion.Euler(sweepangle, currentRotation.eulerAngles.y, currentRotation.eulerAngles.z);

        // Nesnenin rotasyonunu güncelleyin
    ...
    private void MoveForward()
    {
        ...
        if (rb.position.x - en_yakinPos.x < 20f && rb.position.z - en_yakinPos.z < 20f)
                    {
                        movement = new Vector3(horizontalInput, 0, verticalInput) * (speed_force/3) * Time.deltaTime;
                        rb.MovePosition(rb.position + droneTransform.TransformDirection(movement));
                    }
